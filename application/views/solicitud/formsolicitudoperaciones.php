				...
				<div class="form-group">
					...
					<div class="col-md-4">
						<div class="input-group">
							...
							<select name="idasesor" class="form-control input-sm">
								<?php foreach ($asesores as $row) { ?>
								<option value="<?php echo $row['codusuario'] ?>" <?php echo $usuario['codusuario']==$row['codusuario'] ? 'selected="selected"' : '' ?>><?php echo $row['apenom'] ?></option>
								<?php } ?>
							</select>
						</div>
					</div>                         
